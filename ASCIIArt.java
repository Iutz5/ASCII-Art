...
public class ASCIIArt {
    ...
    public static void getPixel (double [][] image) {
        int width = 4;
        int height = 8;
        double pixel = 0;
        char ascii = 0;
        for (int i = 0 ; i < image.length-height ; i = i + height) {
            for (int j = 0 ; j < image[i].length-width; j = j + width) {
                pixel = image[i][j];
                ascii = getChar(averagePixel(image,i,j,width,height));
    ...
    public static char getChar (double pixel) {
        double smallestDiff = Math.abs(pixel - GRAYSCALE_VALUES[0]);
        char match = GRAYSCALE_CHARS.charAt(0);
        for (int i = 0 ; i < GRAYSCALE_VALUES.length ; i++) {
            double difference = Math.abs(pixel - GRAYSCALE_VALUES[i]);
            
            if (difference < smallestDiff) {
                //System.out.println(pixel + " " + GRAYSCALE_VALUES[i] + " " + difference + " " + GRAYSCALE_CHARS.charAt(i));
                smallestDiff = difference;
                match = GRAYSCALE_CHARS.charAt(i);
            }
        }
        return match;
    }

    public static double averagePixel (double [][] image , int top , int left , int width , int height) {
        double sum = 0;
    ...
    public static void main(String[] args) {
        Picture picture = new Picture("Examples/MickeyMouseImage.jpg");
